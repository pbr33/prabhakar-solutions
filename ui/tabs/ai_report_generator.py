...
"""
AI Report Generator — institutional-grade, LLM-powered, 100% real data.
...
REPORT_CATALOG = {
    "Investment Memorandum": {
        "icon": "📋",
        "description": "Full PE investment memo — MOIC, IRR, investment thesis, and committee-ready recommendation.",
    ...
    for sym in symbols[:5]:
        ...
        for art in articles[:per_symbol]:
            if isinstance(art, dict) and art.get("title"):
                lines.append(f"[{sym}] {art['title']} — {art.get('date', '')}")
...
def _llm_call(llm, prompt: str, label: str) -> str:
    try:
        ...
        return f"⚠️ {label} generation failed: {exc}"
...
5. Stress Test Scenarios — propose 3 realistic macro scenarios with estimated P&L impact using actual position sizes
...
2. Top Contributors — explain each top-3 position's outsized return using the actual numbers
3. Key Detractors — explain underperformance drivers and forward outlook
...
2. News Sentiment Analysis — interpret each headline group and infer sector/company sentiment
3. Price Action Analysis — comment on the Return % data above; which positions signal momentum vs. fatigue
4. Key Risks & Tailwinds for the current portfolio
5. Investment Implications — specific, actionable guidance
...
def _dispatch_report(report_type: str, params: dict, pe_portfolio,
    ...
    if report_type == "Investment Memorandum":
        ...
        with st.spinner(f"Fetching live fundamentals for {ticker} from EODHD…"):
...
def render_ai_report_generator_tab(trading_engine=None, pe_portfolio=None, llm=None):
    """World-class AI Report Generator — real data, LLM-powered, production-ready."""
    ...
    if not llm:
        st.warning(
            "⚠️ LLM not configured. Set up Azure OpenAI in **API Configuration** to unlock report generation."
        )
    ...
    main_tabs = st.tabs(["📋 Generate Report", "🕒 Report History", "📈 Usage Analytics"])

    # ── Generate Report ───────────────────────────────────────────────────────
    with main_tabs[0]:

        # Step 1 — report type
        st.markdown("#### 1 · Select Report Type")
        ...
        st.markdown(
            "**Sections:** " + "  ·  ".join(f"`{s}`" for s in meta["sections"])
        )
        st.markdown("---")
        ...
        with st.expander("Advanced Options"):
            custom_instructions = st.text_area(
                ...
                placeholder=(
                    ...
                    "'Include peer comparisons', 'Use conservative tone'…"
                ),
                height=80,
            )
        ...
        if generate_btn and llm:
            ...
            with st.spinner(
                f"Generating **{selected_report}** — fetching live data and calling AI…"
            ):
            ...
            if mark_draft:
                report_content = "⚠️  DRAFT — NOT FOR DISTRIBUTION\n\n" + report_content
...
{report_content}
    ...
    # ── Report History ────────────────────────────────────────────────────────
    with main_tabs[1]:
        st.markdown("#### 🕒 Report History  *(this session)*")
        ...
        if not st.session_state.report_history:
            ...
            for i, rep in enumerate(reversed(st.session_state.report_history)):
                ...
                label = (
                    f"#{real_idx}  ·  {rep['type']}  ·  "
                    f"{rep['generated_at']}  ·  {rep.get('word_count', '?')} words"
                )
...
{rep["content"]}
    ...
    # ── Usage Analytics ───────────────────────────────────────────────────────
    with main_tabs[2]:
        st.markdown("#### 📈 Session Usage Analytics")
